...
# ----------------------
  ...
  it '15. gets all item names associated with all orders' do
    ...
    # order_items = Order.all.map do |order|
    #   order.items.pluck(:name)
    # end
    # names = order_items.flatten
    # ------------------------------------------------------------
...
# ========================
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where(user_id: @user_3.id).order(:created_at).limit(3).last.items.pluck(:name)
...
# ========================
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where.not("order_items.item_id != ?", @item_4.id)
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where(user: @user_2, "order_items.item_id": @item_4)
    # -----------------------------------------------------------
...
# ========================
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select("users.name, count(items.id) as total_item_count").group("users.name").order("users.name")
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: :order_items).select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.item_id) as avg_item_cost").group("orders.id").order("users.name DESC, avg_item_cost ASC")
    # ---------------------------------------------------------------
